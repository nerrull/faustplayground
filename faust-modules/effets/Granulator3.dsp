...
taille = hslider("taille[BELA: ANALOG_1]", 100, 4, 1000, 0.001 );        // Size in millisecondes
...
random_trig(n, click) = multirandom(n),0: select2(click):_;
...
unGrain(input, clk) = (linrwtable( wf , rindex) : *(0.2 * EnvGrain))
    with {
        ...
        //rindex = %(int(rampe2(speed, clk)) + posTabl + int(size * decal), size);
  		random_offset =random_number_trigger(clk)*taille*random_spread;
